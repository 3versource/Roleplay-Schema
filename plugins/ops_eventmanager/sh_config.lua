...
PLUGIN.Ops.EventManager.Config.CategoryIcons = {
...
PLUGIN.Ops.EventManager.Config.TagColours = {
...
PLUGIN.Ops.EventManager.Config.Events = {
	["empty"] = {
		Cat = "hidden",
		Prop = {},
		NeedUID = false,
	...
	--[[
		-- impulse.MakeBigExplosion isnt implemented.
	...
	["chat"] = {
		...
		Do = function(prop, uid)
			for v,k in pairs(player.GetAll()) do
				k:ChatNotify(prop["message"])
			end
		end
	},
	["cineintro"] = { -- No clue what this is but i guess it should work?
		Cat = "ui",
		Prop = {
			["title"] = "Event Name"
		},
		...
		Clientside = false,
		Do = function(prop, uid)
			PLUGIN.CinematicIntro(prop["title"])
		end
	},
	["screenfade"] = {
		Cat = "ui",
	...
	["itemspawn"] = {
		...
		Clientside = false,
		Do = function(prop, uid)
			ix.item.Spawn(prop["itemclass"], prop["pos"])
		end
	},
	--[[
	...
	MakeDropshop needs to be defined.
	...
	["createscene"] = {
		...
		Clientside = true,
		Do = function(prop, uid)
			if not PLUGIN.Ops.EventManager.Scenes then
				return
			end
			...
			PLUGIN.Ops.EventManager.Scenes[uid] = {prop}
		end
	},
	["playscene"] = {
		Cat = "scene",
