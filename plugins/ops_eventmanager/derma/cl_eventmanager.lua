...
function PANEL:Init()
	...
	menuFile:AddOption("New sequence", function()
		Derma_StringRequest("impulse ops",
			...
			nil, 
			function(text)
				PLUGIN.Ops.EventManager.Sequences[text] = {
					Name = text, 
					FileName = text,
					Events = {}, 
					VersionHash = PLUGIN.Ops.EventManager.GetVersionHash()
				}
	...
	menuFile:AddOption("Load sequence", function()
		...
		function browser:OnSelect(path)
			local result, msg = PLUGIN.Ops.EventManager.SequenceLoad(path)
...
function PANEL:SetupPlayer(sequences)
	...
	for v,k in pairs(sequences) do
		...
		function sequence:Paint(w, h)
			...
			if PLUGIN.Ops.EventManager.GetSequence() and PLUGIN.Ops.EventManager.GetSequence() == k then
