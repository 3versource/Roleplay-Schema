...
function PANEL:SetSequence(key, data)
	...
	self.main:SetTooltip("Version hash: "..(PLUGIN.Ops.EventManager.Sequences[key].VersionHash or "outdated"))
	...
	function newSeq:DoClick()
		local id = table.insert(PLUGIN.Ops.EventManager.Sequences[key].Events, {
			Type = "empty",
			Prop = {},
			UID = nil,
			Delay = 0
		...
		table.Merge(PLUGIN.Ops.EventManager.Sequences[key].Events[id].Prop, PLUGIN.Ops.EventManager.Config.Events["empty"].Prop)

		panel:AddEvent(id, PLUGIN.Ops.EventManager.Sequences[key].Events[id])
	end
	...
	function remSeq:DoClick()
		PLUGIN.Ops.EventManager.Sequences[key] = nil
	...
	function saveSeq:DoClick()
		if not PLUGIN.Ops.EventManager.Sequences[key].FileName then
			Derma_StringRequest("impulse", "Enter sequence file name:", nil, function(name)
				PLUGIN.Ops.EventManager.Sequences[key].FileName = name
				PLUGIN.Ops.EventManager.SequenceSave(key)
	...
	function uploadSeq:DoClick()
		PLUGIN.Ops.EventManager.SequencePush(key)
	end
...
function PANEL:AddEvent(id, eventdata)
	...
	function event:Paint(w, h)
		...
		local colTag = PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag
		if colTag and PLUGIN.Ops.EventManager.Config.TagColours[colTag] then
			draw.RoundedBox(0, 0, 0, w, h, ColorAlpha(PLUGIN.Ops.EventManager.Config.TagColours[colTag], 40))
		end

		draw.RoundedBox(0, 0, h-2, w, 2, Color(100, 100, 100, 150))

		local curEvents = PLUGIN.Ops.EventManager.GetCurEvents()
		...
		if not curEvents[id + 1] and (PLUGIN.Ops.EventManager.GetSequence() and panel.Sequence == PLUGIN.Ops.EventManager.GetSequence()) then
			PLUGIN_OpsEM_CurEvents[id] = CurTime()
		end
	...
	event.etypeicon:SetImage(PLUGIN.Ops.EventManager.Config.CategoryIcons[PLUGIN.Ops.EventManager.Config.Events[eventdata.Type].Cat])
	...
	function event.edelay:OnValueChanged(new)
		if tonumber(new) then
			...
			PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Delay = realNew
		end
	end
	...
	function event.eremove:DoClick()
		table.remove(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, id)
		panel.Dad:ReloadSequences()
	end
	...
	function event.etypebtn:DoClick()
		...
		for v,k in pairs(PLUGIN.Ops.EventManager.Config.Events) do
			...
			if not cats[k.Cat] then
				...
				p:SetIcon(PLUGIN.Ops.EventManager.Config.CategoryIcons[k.Cat])
				cats[k.Cat] = c
			end

			local parent = cats[k.Cat]
			parent:AddOption(v, function()
				PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Type = v
				PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop = {}
				table.Merge(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop, PLUGIN.Ops.EventManager.Config.Events[v].Prop)
				panel.Dad:ReloadSequences()
			end)
		end
	...
	if table.Count(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop) == 0 then
	...
	function event.eprop:DoClick()
		...
		panel.Dad.Properties:SetTable(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop, function(key, val)
			PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop[key] = val
		end)
		panel.Dad.Properties:SetTitle(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].Type.."("..id..") properties")
		...
		for v,k in pairs(PLUGIN.Ops.EventManager.Config.TagColours) do
			...
			function a:DoClick()
				PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag = v
			end
		end
		...
		function noCol:DoClick()
			PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag = nil
		end
		...
		notes:SetValue(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaNotes or "")

		function notes:OnValueChange()
			PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaNotes = self:GetValue()
		end
	end
	...
	event.euid:SetText(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID or "")
	...
	function event.euid:OnValueChange(new)
		...
		if new == "" then
			PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID = nil
			return
		end

		PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID = new
	end

	function event.euid:PaintOver(w, h)
		if PLUGIN.Ops.EventManager.Config.Events[eventdata.Type].NeedUID and not self:IsEditing() and string.Trim(self:GetText(), " ") == "" then
	...
	function event.euid:GetAutoComplete(text)
		...
		for v,k in pairs(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events) do
			if v == id then
				continue
			end
	...
	local function moveToCustomSlot()
		Derma_StringRequest("impulse ops", "Enter where you wish to move this event to:", "", function(slot)
			...
			local me = PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events[id]
			local oldId = id
			local size = table.Count(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events)
			slot = math.Clamp(math.floor(slot), 1, size)

			table.remove(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, id)
			table.insert(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, slot, me)
	...
	function event.emup:DoClick()
		...
		table.remove(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, id)
		table.insert(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, math.Clamp(oldId - 1, 1, size), me)
	...
	function event.edupe:DoClick()
		...
		table.insert(PLUGIN.Ops.EventManager.Sequences[panel.Sequence].Events, math.Clamp(oldId + 1, 1, size), copy1(me))
